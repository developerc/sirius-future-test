...
const Range = styled.input`
  ...
  background: linear-gradient(to right, yellow 0%, yellow 55%, #fff 55%, #fff 100%);
...
const getSliderVal = (idVal) => {
  ...
  export const Slider: FunctionComponent<RangeProps> = ({minVal, maxVal, idVal}) => {
    ...
    return (
        <>
        <Range className={styles.slider}  type="range" min={minVal} max={maxVal} id={idVal} onChange = {() => changeSlider(minVal, maxVal, idVal)}  ></Range>        
        </>
    );
  }
